...
function fmtDate(d: string) { const x = new Date(d + "T12:00:00"); return `${tr(DAY_L[(x.getDay() + 6) % 7])} ${x.getDate()}.${x.getMonth() + 1}`; }
  ...
  return (
    <Sheet open={open} onClose={onClose} title="Bjóða vakt til skipta">
      ...
      <Field label="Skilaboð">
        <TextInput style={[inputStyle(), { minHeight: 90, textAlignVertical: "top" }]} multiline value={note} onChangeText={setNote} placeholder={tr("t.d. Býð laugardagsvaktina 11:30–23:00 — Wiktoria getur tekið hana")} placeholderTextColor={colors.ink3} />
      </Field>
  ...
  return (
    <Sheet open={open} onClose={onClose} title="Leiðrétta tíma">
      ...
      <View style={{ flexDirection: "row", gap: 10 }}>
        <View style={{ flex: 1 }}><Field label="Rétt inn"><TextInput style={inputStyle()} value={inT} onChangeText={setIn} placeholder={tr("11:30")} placeholderTextColor={colors.ink3} keyboardType="numbers-and-punctuation" /></Field></View>
        <View style={{ flex: 1 }}><Field label="Rétt út"><TextInput style={inputStyle()} value={outT} onChangeText={setOut} placeholder={tr("23:05")} placeholderTextColor={colors.ink3} keyboardType="numbers-and-punctuation" /></Field></View>
      </View>
      <Field label="Skýring"><TextInput style={[inputStyle(), { minHeight: 70, textAlignVertical: "top" }]} multiline value={reason} onChangeText={setReason} placeholder={tr("Gleymdi að stimpla út…")} placeholderTextColor={colors.ink3} /></Field>
      <Btn title="Senda beiðni" size="lg" loading={busy} onPress={send} />
    </Sheet>
  );
}
...
export function CantSheet({ open, onClose, onDone, shiftLabel }: { open: boolean; onClose: () => void; onDone?: () => void; shiftLabel: string }) {
  ...
  return (
    <Sheet open={open} onClose={onClose} title="Get ekki mætt">
      ...
      <Field label="Ástæða"><TextInput style={[inputStyle(), { minHeight: 70, textAlignVertical: "top" }]} multiline value={reason} onChangeText={setReason} placeholder={tr("Veik(ur)…")} placeholderTextColor={colors.ink3} /></Field>
