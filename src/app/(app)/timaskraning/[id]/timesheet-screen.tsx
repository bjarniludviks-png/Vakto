...
export default function EmployeeTimesheet({ id, name, initial, initialMissed, needsMigration, from: f0, to: t0 }: { id: string; name: string; initial: PunchRow[]; initialMissed: MissedShift[]; needsMigration: boolean; from: string; to: string }) {
  ...
  return (
    <>
      ...
      {mig && <div className="ai" style={{ margin: "16px 0 0" }}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><circle cx="12" cy="12" r="9" /><path d="M12 8h.01M11 12h1v4h1" /></svg><div className="x">{t("Samþykki stimplana er ekki virkt í þessu fyrirtæki — hafðu samband við VAKTO.")}</div></div>}
