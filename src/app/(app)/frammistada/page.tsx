...
export default async function FrammistadaPage() {
  ...
  const [{ empty }, perf, staffing, history] = await Promise.all([
    getCompanyData(), getPerformance(), getStaffingPattern(iso(from), iso(to)), getPerfHistory(6),
  ]);
  return <PerformanceScreen empty={empty} live={perf.live} perf={perf} staffing={staffing} history={history} />;
}
